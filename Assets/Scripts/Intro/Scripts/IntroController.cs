...
public class IntroController : MonoBehaviour
{
    ...
    {
        float currentLoadingTextTime = 
            introCard.data.loadingTextRefreshRate;
        if (introCard == null)
        {
        ...
        if (generatedIntroCard != null)
        {
            while (
                generatedIntroCard != null && 
                generatedIntroCard.activeSelf && 
                !stopLoadingTextCoroutine
            ) //while intro card is active, keep updating loadingTexts
            {
    ...
    {
        introCard ??= NewIntroCard
            (topicName ?? "Narrator forgot the actual topic again..."
        );
        if (introCard != null)
        {
            if (introCard.data == null)
                introCard.data = 
                    cardDatas[UnityEngine.Random
                        .Range(0, cardDatas.Length)];

            generatedIntroCard = Instantiate(
                introCard.data.introCardContainer.gameObject
            );
            generatedIntroCard.transform.SetParent(transform, true);
            generatedIntroCard.SetActive(true);
            audioSource.clip = introCard.GetRandomMusicClip();
            introCard.SetCardInstance(
                generatedIntroCard.GetComponent<CardObject>()
            );
            string newloadingText = introCard.GetNextLoadingText();
            introCard.UpdateLoadingText(newloadingText);
            Sprite newBackgroundImage = introCard
                .GetRandomBackgroundImage();
            introCard.UpdateBackgroundImage(newBackgroundImage);
            introCard.topicName = string.IsNullOrEmpty(topicName) ? 
                introCard.topicName = introCard.data.noTopicsText : 
                topicName;
