...
def tts():
    ...
    print(result[1])
    audioPath = result[1].replace("/private/", "file:///")
    string_json = '{"status": "%s", "audioPath": "%s"}' % (result[0], audioPath)
    json_output = json.loads(string_json)
    return json_output

@app.route("/edge-tts/", methods=['POST'])
def edge_tts():
    content_type = request.headers.get('Content-Type')
    json_input = json.loads("{}")
    if ('application/json' in content_type):
